...
```mermaid
flowchart TB
    UI["前端展示层<br/>Vite + React<br/>ChatPanel · MapPanel · TimelinePanel<br/>ToolCallsPanel · SafetyPanel · TracePanel"]
    API["FastAPI Gateway<br/>/api/plan · /api/chat · /api/session<br/>SSE · WebSocket · /api/tools"]
    ORCH["Agent Orchestrator<br/>会话恢复 · 状态传递 · 流式事件"]

    PA["Planning Agent<br/>规划编排 Agent<br/>需求理解 · 多步规划 · 规划草案"]
    RA["Review Agent<br/>安全审查 Agent<br/>风险检查 · 权限审查 · 质量校验"]
    DRAFT["规划草案<br/>TravelState · 工具轨迹<br/>来源引用 · 风险信息"]
    DECISION["审查结论<br/>通过 · 需要修改 · 拒绝 · HITL"]
    OUT["最终输出层<br/>Markdown 行程 · 地图数据<br/>时间线 · 预算 · 来源与风险"]

    STATE["LangGraph TravelState<br/>9 阶段状态机<br/>偏好 → 约束 → 搜索 → 路线 → 天气 → 预算 → 合成 → 安全 → 输出"]
    SKILLS["Skills 层<br/>Preference · Constraint · MapSearch · GuideSearch<br/>Route · Weather · Budget · Synthesis · Safety · HITL"]
    CONTEXT["ContextManager<br/>原始数据 · 压缩摘要 · 约束上下文 · 当前 Prompt"]
    MEMORY["MemoryManager<br/>会话记忆 · 语义记忆 · 情景记忆 · 程序记忆"]
    SOURCES["外部工具链与数据源<br/>高德 MCP · OpenTripMap · OSRM · Open-Meteo<br/>JustOneAPI · 小红书攻略搜索"]
    STORAGE["数据存储与缓存<br/>文件系统 · PostgreSQL · ChromaDB · mem0"]
    SAFETY["四层安全防护<br/>输入侧 · 工具侧 · 日志侧 · 动作侧"]
    EVAL["评测与可观测<br/>Langfuse · RACE · DoVer · AgentWorld · FACT<br/>failures.jsonl · AGENTS.md 自进化"]
    ...
    classDef state fill:#EAF8F8,stroke:#2EC4B6,color:#0A2463,stroke-width:1.5px;
    ...
    classDef data fill:#FFF8EF,stroke:#FF9F1C,color:#63351F,stroke-width:1.3px;
    class UI,API,ORCH app;
    class PA,RA,DRAFT,DECISION agent;
    class STATE,SKILLS,CONTEXT,MEMORY state;
    class OUT output;
    class SAFETY security;
    class SOURCES,STORAGE,EVAL data;
```
